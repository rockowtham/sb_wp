...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        function add_importer_section() {
            ...
            $this->parent->sections[] = array(
                ...
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'redux-framework' ) ).'</p>',
	...
	if ( !function_exists( 'wbc_after_theme_options' ) )
	{
		...
		function wbc_after_theme_options( $demo_active_import , $demo_data_directory_path )
		{
			...
			if( $current_demo == 'CarSpot Dealer')
			{
				$home = get_page_by_title( 'Home 6' );
				...
				if (!wp_get_nav_menu_object('carspot Primary Menu'))
				{
					$menu_id = wp_create_nav_menu('carspot Primary Menu'); //create the menu
					$locations = get_theme_mod('nav_menu_locations'); //get the menu locations
					$locations['main_menu'] = $menu_id; //set our new menu to be the main nav
					set_theme_mod('nav_menu_locations', $locations); //update 
				}
				else
				{
					$menu_name = "carspot Primary Menu";
			...
			if( $current_demo == 'CarSpot Classified')
			{
			...
			if( $current_demo == 'CarSpot RTL')
			{
				$home = get_page_by_title( 'الصفحة الرئيسية' );
				update_option( 'page_on_front', $home->ID );
				update_option( 'show_on_front', 'page' );
			...
			if( $current_demo == 'CarSpot Bikes')
			{
				$home = get_page_by_title( 'اHome 4' );
				update_option( 'page_on_front', $home->ID );
				update_option( 'show_on_front', 'page' );
			...
			if( $current_demo == 'CarSpot Services')
			{
				$home = get_page_by_title( 'اHome' );
				update_option( 'page_on_front', $home->ID );
				update_option( 'show_on_front', 'page' );
