...
add_action( 'wp_ajax_sb_cart', 'carspot_packages_cart' );
add_action( 'wp_ajax_nopriv_sb_cart', 'carspot_packages_cart' );

function carspot_packages_cart()
{
...
function carspot_packages_pricing_custom_js() {
	...
	?><script type='text/javascript'>
		jQuery( document ).ready( function() {
			jQuery( '#carspot_metaboxes_packages' ).hide();
			jQuery( '.options_group.pricing' ).addClass( 'show_if_carspot_packages_pricing' ).show();
			jQuery( '.options_group.pricing' ).addClass( 'show_if_carspot_packages_pricing' ).show();
			
			jQuery('#product-type').on('change', function()
			{
				if( jQuery(this).val() == 'carspot_packages_pricing' )
				{
					jQuery( '#carspot_metaboxes_packages' ).show();
...
add_action( 'admin_footer', 'carspot_packages_pricing_custom_js' );
// adding product type
function register_carspot_packages_pricing_product_type() {
	class WC_Product_carspot_packages_pricing extends WC_Product {
		public function __construct( $product ) {
			$this->product_type = 'carspot_packages_pricing';
		parent::__construct( $product );
		}
	}
}
add_action( 'init', 'register_carspot_packages_pricing_product_type' , 0);
function add_simple_rental_product( $types ){
	// Key should be exactly the same as in the class product_type parameter
	$types[ 'carspot_packages_pricing' ] = __( 'Carspot Packages Based' );
	return $types;

}
add_filter( 'product_type_selector', 'add_simple_rental_product',0 );

function carspot_packages_pricing_hide_attributes_data_panel( $tabs) {
	// Other default values for 'attribute' are; general, inventory, shipping, linked_product, variations, advanced
	$tabs['attribute']['class'][] = 'hide_if_carspot_packages_pricing';
	$tabs['shipping']['class'][] = 'hide_if_carspot_packages_pricing';
	$tabs['linked_product']['class'][] = 'hide_if_carspot_packages_pricing';
	$tabs['advanced']['class'][] = 'hide_if_carspot_packages_pricing';
	return $tabs;
}
add_filter( 'woocommerce_product_data_tabs', 'carspot_packages_pricing_hide_attributes_data_panel' );