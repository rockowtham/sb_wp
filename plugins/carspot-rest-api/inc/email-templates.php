...
if( !function_exists('carspotAPI_get_notify_on_ad_message' ) )
{
	function carspotAPI_get_notify_on_ad_message($pid = '', $msg_receiver_id = '',$ad_message = '',$name = '')
	{
		global  $carspotAPI;
		global  $carspot_theme;
		if( isset($carspot_theme['sb_send_email_on_message']) && $carspot_theme['sb_send_email_on_message'] )
		{
...
if( !function_exists('carspotAPI_email_on_new_user' ) )
{
	function carspotAPI_email_on_new_user($user_id, $social = '')
	{
		...
		if( isset( $carspot_theme['sb_new_user_email_to_admin'] ) && $carspot_theme['sb_new_user_email_to_admin'] )
		{
...
function carspotAPI_forgot_pass_email_link($email = '')
{
			...
			if( isset( $carspot_theme['sb_forgot_password_message'] ) &&  $carspot_theme['sb_forgot_password_message'] != "" )
			{
				...
				if( $mail )
				{
					...
					$success = true;
					$message =   __( 'Password reset sent, check your email inbox', 'carspot-rest-api' );	
...
function carspotAPI_email_ad_rating($pid,$sender_id, $rating, $comments)
{
	...
	global $carspot_theme;
	$from	=	get_bloginfo( 'name' );
	if( isset( $carspot_theme['ad_rating_email_from'] ) && $carspot_theme['ad_rating_email_from'] != "" )
	{
		$from	=	$carspot_theme['ad_rating_email_from'];
	}
	$headers = array('Content-Type: text/html; charset=UTF-8',"From: $from" );
	if( isset( $carspot_theme['ad_rating_email_message'] ) &&  $carspot_theme['ad_rating_email_message'] != "" )
	{
		
		$author_id = get_post_field ('post_author', $pid);
		$user_info = get_userdata($author_id);
		
		$subject = $carspot_theme['ad_rating_email_subject'];
		...
		$to = $user_info->user_email;
		$body = str_replace($msg_keywords, $msg_replaces, $carspot_theme['ad_rating_email_message']);
		$mail  = wp_mail( $to, $subject, $body, $headers );
		if($mail)
		{
			return 'sent';
...
function carspotAPI_email_ad_rating_reply($pid,$receiver_id, $reply, $rating, $rating_comments)
{
	...
	global $carspot_theme;
	$from	=	get_bloginfo( 'name' );
	if( isset( $carspot_theme['ad_rating_reply_email_from'] ) && $carspot_theme['ad_rating_reply_email_from'] != "" )
	{
		$from	=	$carspot_theme['ad_rating_reply_email_from'];
	}
	$headers = array('Content-Type: text/html; charset=UTF-8',"From: $from" );
	if( isset( $carspot_theme['ad_rating_reply_email_message'] ) &&  $carspot_theme['ad_rating_reply_email_message'] != "" )
	{
		
		$author_id = get_post_field ('post_author', $pid);
		$user_info = get_userdata($author_id);
		
		$subject = $carspot_theme['ad_rating_reply_email_subject'];
		...
		$receiver_info = get_userdata($receiver_id);
		$to = $receiver_info->user_email;
		$body = str_replace($msg_keywords, $msg_replaces, $carspot_theme['ad_rating_reply_email_message']);
		wp_mail( $to, $subject, $body, $headers );
	}
}
}