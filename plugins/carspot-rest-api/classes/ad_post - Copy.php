...
function carspotAPI_post_ad_get( $request )
{
	...
	$message = '';
	if( !$carspot_theme['admin_allow_unlimited_ads'] ){$message = carspotAPI_check_ads_validity();}
	if ( !is_super_admin( get_current_user_id() ) ){$message = carspotAPI_check_ads_validity();}
	if( $message != "" ){ return array( 'success' => false, 'data' => '' , 'message' => $message ); }
	...
	if( $is_update != "" ){ $pid = (int)$is_update; }
	else if( get_post_status ( $pid ) && $pid != ""){ $pid = (int)$pid; }
	else
	{
	...
	$package_type 				= ( isset($carspot_theme['carspot_package_type']) && $carspot_theme['carspot_package_type'] == 'category_based') ? $carspot_theme['carspot_package_type'] : 'package_based';
	...
	if( isset($dynamicData) && count($dynamicData) > 0 && $dynamicData != "" && $form_type != 'no') { $data['fields'] = $dynamicData; }
...
$data['fields'][] = carspotAPI_getPostAdFields('checkbox', 'ad_features', 'ad_features', 0, __("Features", "carspot-rest-api"),'','','3', true, '', $is_update);	
	...
	if( $pid != "" ){ $images = carspotAPI_get_ad_image($pid, -1, 'thumb', false); }
	...
	if($is_update != "" ) { $bump_ad_is_show = true; }
	if($is_update == "" ) { $bump_ad_is_show = false; }			
	...
	if( $featured_ad_is_show )
	{
		...
		$featured_ad_checkbox_arr	= array("main_title" => "", "field_type" => 'checkbox', "field_type_name" => 'ad_featured_ad',"field_val" => "", "field_name" => "", "title" => $featured_ad_title, "values" => "", "has_page_number" => "3", "is_required" => false, "has_cat_template" => false);
		
	 	$data['profile']['pricing']['package_based']['featured_ad'] = $featured_ad_checkbox_arr;			
	}

	$is_redirect = ( isset($carspot_theme['carspot_package_type']) && $carspot_theme['carspot_package_type'] == 'category_based') ? true : false;
...
if (!function_exists('carspotAPI_getPostAd_adons'))
{
    function carspotAPI_getPostAd_adons($show_on = 'both', $is_edit = 0) {
        ...
        $lists_array =  $list_array = array(); $lists = array();
						...
						if($r['titles'] != "" && $r['slugs'] != ""){	
							...
							foreach( $varArrs as $v )
							{
									...
									if( $r['types'] == 3 ) 
									{
										...
										$is_checked = false;
										if( !is_array($nameValue) && $nameValue == $v) { $is_checked = true;}
										else
										{
