...
add_action( 'rest_api_init', 'carspotAPI_phone_verification_hook', 0 );
function carspotAPI_phone_verification_hook() {
    register_rest_route(
        		'carspot/v1', '/profile/phone_number/', array(
				'methods'  => WP_REST_Server::READABLE,
				'callback' => 'carspotAPI_verification_system',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    );
} 

if ( ! function_exists( 'carspotAPI_verification_system' ) ) 
{
	function carspotAPI_verification_system()
	{
		global $carspotAPI;
		...
		{
			$message = __('Please go to edit profile and update valid phone number +CountrycodePhonenumber.','carspot-rest-api');
		...
		if( isset( $carspotAPI['sb_resend_code'] ) && $carspotAPI['sb_resend_code'] != "" && get_user_meta($user_id, '_ph_code_', true) != "" )
		{
			...
			if( $carspotAPI['sb_resend_code'] > $differenceInSeconds )
			{
				...
				$success = false;
				$message =  __( "You can't resend the verification code before", 'carspot-rest-api' )." ".$after_seconds.' '. __( "seconds.", 'carspot-rest-api' );
				$data['code'] = $code;
		...
		$res	=	carspotAPI_send_sms($ph, $code);
		if( isset($res) && count((array)$res) > 0 && @$res->sid && $message_sent)
		{
			...
			$success = true;
			$message = __( "Verification code has been sent.", "carspot-rest-api" );
		}
		else
		{		
			$success = false;
			$message = __( "Server not responding.", "carspot-rest-api" );
			update_user_meta( $user_id, '_sb_is_ph_verified', '0' );
		}	
			$timeFirst  = strtotime(get_user_meta($user_id, '_ph_code_date_', true));
			$timeSecond = strtotime(date('Y-m-d H:i:s'));
			...
			if( $sb_resend_code > $differenceInSeconds )
			{
				...
				$resend_text =  __( "You can't resend the verification code before", 'carspot-rest-api' )." ".$after_seconds.' '.__( "seconds.", 'carspot-rest-api' );
			}
		$data['code'] = $code;
...
add_action( 'rest_api_init', 'carspotAPI_verification_code_hook', 0 );
function carspotAPI_verification_code_hook() {
    register_rest_route(
        		'carspot/v1', '/profile/phone_number/verify/', array(
				'methods'  => WP_REST_Server::EDITABLE,
				'callback' => 'carspotAPI_verification_code',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    );
} 
if ( ! function_exists( 'carspotAPI_verification_code' ) ) 
{
	function carspotAPI_verification_code($request)
	{
		$user 	 	= wp_get_current_user();
		$user_id 	= $user->ID;
		...
		{
			$success = false;
			$message = __( "Code not found.", "carspot-rest-api" );
		}
		else if( $code == $saved )
		{
			...
			$success = true;
			$message = __( "Phone number has been verified.", "carspot-rest-api" );
		}
		else
		{
			$success = false;
			$message = __( "Invalid code that you entered.", "carspot-rest-api" );
		}
...
if ( !function_exists( 'carspotAPI_send_sms' ) ) 
{
	function carspotAPI_send_sms($receiver_ph, $code)
	{
		global $carspotAPI;
		...
		if( isset( $twl_data  ) && $twl_data != "" )
		{
			...
			{
				$message	= __('Your verification code is', 'carspot-rest-api') . " " . $code;
				try {
			...
			{
				return __('Something went wrong please check later', 'carspot-rest-api') . " " . $code;	
			}
		}
		else
		{
