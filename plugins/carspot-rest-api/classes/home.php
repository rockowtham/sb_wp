...
if (!function_exists('carspotAPI_homeScreen_get'))
{
	function carspotAPI_homeScreen_get()
	{
			...
			if(isset( $array_sortable ) && $array_sortable > 0 )
			{
				...
				foreach( $arrays as $key => $val )
				{
					...
					if( $key == "body_type" && $val != "")
					{
						...
						$body_type_icon_txt = (isset( $carspotAPI['api_home_body_type_section_title'] ) ) ? $carspotAPI['api_home_body_type_section_title'] : '';
		...
		$data['extra']['ad_post'] 	= $carspotAPI['api_home_body_type_ad_post'];
	...
	function carspotAPI_home_adsLayouts($type = '')
	{
		...
		if( $type == 'latest' )
		{
			...
			if( isset( $carspotAPI['latest_on_home'] ) && $carspotAPI['latest_on_home'] == 1 )
			{
				...
				$latestAds = carspotApi_featuredAds_slider( '', 'active', '', $latestAdsCount, $latest_termID, 'publish', '' , 'only_simple');	
			...
			while ( $the_query->have_posts() ) {
				...
				$adsArr[$cats_name][] = array
					(
		                ...
		                'ad_warranty' => get_post_meta($ad_id, '_carspot_ad_warranty1', true),
	                    'ad_user_name'  => get_post_meta($ad_id, '_carspot_ad_user_name', true),
...
if (!function_exists('carspotAPI_appMenu_settings'))
{
	...
	{		
		...
		$data_menu['home_menu'] 	= carspotAPI_settingsMenu();
