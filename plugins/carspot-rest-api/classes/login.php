	...
	function carspotAPI_loginMe_post($request)
	{
		...
		if( $type == 'social' )
		{
			...
			if( $user )
			{
				...
				$profile_arr['dealer_cinfirm_dialog'] 	= carspotAPI_dealer_cinfirmDialog($user->ID);
				$response = array( 'success' => true, 'data' => $profile_arr, 'message'  => __("Login Successful", "carspot-rest-api")  );					
			}
			else
			{
				$response = array( 'success' => true, 'data' => '', 'message'  => __("Something went wrongssss", "carspot-rest-api")  );
			}
		...
		else
		{
			...
			if( isset( $carspotAPI['sb_new_user_email_verification'] ) && $carspotAPI['sb_new_user_email_verification'] )
			{
				...
				return array( 'success' => false, 'data' => $profile_arr, 'message'  => __("Please verify your email address to login.", "carspot-rest-api") );
				}
			}
			$response = array( 'success' => true, 'data' => $profile_arr, 'message'  => __("Login Successful", "carspot-rest-api")  );
...
if (!function_exists('carspotAPI_loginMe_get'))
{
	...
	{		
		...
		$data['popup_forgot_title']			=  __("Forgot Your Password?", "carspot-rest-api");
		$data['popup_forgot_button_text']			=  __("Reset My Account", "carspot-rest-api");
