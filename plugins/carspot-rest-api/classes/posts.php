...
add_action( 'rest_api_init', 'carspotAPI_hook_for_getting_posts', 0 );
function carspotAPI_hook_for_getting_posts() {
    register_rest_route(
        'carspot/v1', '/posts/', array(
				'methods'  => WP_REST_Server::READABLE,
				'callback' => 'carspotAPI_get_posts_get',
...
{
	function carspotAPI_get_posts_get( $request )
	{
		...
		if ( $posts->have_posts() ) {
			while ( $posts->have_posts() ) 
			{
				...
				$arr['details'] = carspotAPI_words_count(get_the_excerpt(), 90);
				$post_data[] = $arr;
			}
	...
	$data['pagination'] = array("max_num_pages" => (int)$posts->max_num_pages, "current_page" => (int)$paged, "next_page" => (int)$nextPaged, "increment" => (int)$posts_per_page , "current_no_of_ads" =>  (int)($posts->found_posts), "has_next_page" => $has_next_page );		
		...
		$extra['page_title'] = __("Blog", "carspot-rest-api" );
...
add_action( 'rest_api_init', 'carspotAPI_hook_for_getting_post', 0 );
function carspotAPI_hook_for_getting_post() {
    register_rest_route(
        'carspot/v1', '/posts/detail/', array(
				'methods'  => WP_REST_Server::EDITABLE,
				'callback' => 'carspotAPI_get_post_get',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    );
}  

if( !function_exists('carspotAPI_get_post_get' ) )
{
	function carspotAPI_get_post_get( $request )
	{
		$json_data 		= $request->get_json_params();		
		...
		$wrapHTML = '<span style=" color:#a0a0a0;line-height:24px;">'.$post->post_content.'</span>';
		...
		$arr['comments']  	= carspotAPI_get_post_comments( $post_id, $post->comment_count );
		$data['post'] 		= $arr;
		...
		$extra['comment_form']['title'] 		=  __("Your Comment", "carspot-rest-api" );
		$extra['comment_form']['textarea'] 		=  __("Post your comment", "carspot-rest-api" );
		$extra['comment_form']['btn_submit'] 	=  __("Submit", "carspot-rest-api" );
		$extra['comment_form']['btn_cancel'] 	=  __("Cancel", "carspot-rest-api" );
		$extra['comment_form']['btn_reply'] 	=  __("Reply", "carspot-rest-api" );
		
		$extra['comment_form']['popup']['title'] 		=  __("Comment reply", "carspot-rest-api" );
		$extra['comment_form']['popup']['textarea'] 	=  __("Post your reply", "carspot-rest-api" );
		$extra['comment_form']['popup']['btn_submit'] 	=  __("Reply", "carspot-rest-api" );		
		$extra['login_text'] = __("Please login first to post comment.", "carspot-rest-api");
		$extra['empty_text'] = __("Please write something.", "carspot-rest-api");
...
if( !function_exists('carspotAPI_get_post_comments' ) )
{
	function carspotAPI_get_post_comments( $post_id = '', $comments_count = '' )
	{
		...
		$parent_comments = carspotAPI_parent_comment_counter($post_id);
		...
		if( count( $comments ) > 0 ) {
			foreach($comments as $comment) {	
				$arr['blog_id'] 			= $post_id;
				$arr['img'] 				= carspotAPI_user_dp( $comment->user_id );				
				$arr['comment_id'] 			= $comment->comment_ID;
				$arr['comment_author'] 		= $comment->comment_author;
				$arr['comment_content'] 	= $comment->comment_content;
				$arr['comment_date'] 		= carspotAPI_timeago($comment->comment_date);
				$arr['comment_parent'] 		= $comment->comment_parent;
				$arr['comment_author_id'] 	= $comment->user_id;
				$arr['reply_btn_text'] 		= __("Reply", "carspot-rest-api");
				
				$replies                = carspotAPI_get_comment_replys($comment->comment_ID, $post_id);
		...
		/*else
		{
			$carray[]  = __('No Comment Found','carspot-rest-api' );
		}*/
...
if( !function_exists('carspotAPI_get_comment_replys' ) )
{
	function carspotAPI_get_comment_replys($comment_id = '',$post_id = '')
	{
		...
		if( count( $rcomments ) > 0 )
		{
			...
			{	
				...
				$rarr['img'] 				= carspotAPI_user_dp( $rcomment->user_id );	
				...
				$rarr['comment_date'] 		= carspotAPI_timeago($rcomment->comment_date);
				$rarr['comment_parent'] 	= $rcomment->comment_parent;
				$rarr['comment_author_id'] 	= $rcomment->user_id;
				$rarr['reply_btn_text'] 	= __("Reply", "carspot-rest-api");
...
add_action( 'rest_api_init', 'carspotAPI_hook_for_getting_comments', 0 );
function carspotAPI_hook_for_getting_comments() {

    register_rest_route(
        'carspot/v1', '/posts/comments/', array(
				'methods'  => WP_REST_Server::READABLE,
				'callback' => 'carspotAPI_get_post_comments_api',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    );
	
    register_rest_route(
        'carspot/v1', '/posts/comments/get/', array(
				'methods'  => WP_REST_Server::EDITABLE,
				'callback' => 'carspotAPI_get_post_comments_api',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    ...
    register_rest_route(
        'carspot/v1', '/posts/comments/', array(
				'methods'  => WP_REST_Server::EDITABLE,
				'callback' => 'carspotAPI_get_post_comments_submit_api',
				'permission_callback' => function () { return carspotAPI_basic_auth();  },
        	)
    );	
}  

if( !function_exists('carspotAPI_get_post_comments_submit_api' ) )
{
	function carspotAPI_get_post_comments_submit_api( $request  )
	{
		...
		if( $comment_id )
		{
			$arr['comments']  = carspotAPI_get_post_comments( $post_id );
			$success = true;
			
			$status = wp_get_comment_status( $comment_id );
			if ( $status == "approved" ) {
			 $message = __("Comment Posted Successfully.", "carspot-rest-api");
			}
			else
			{
				$message = __("Comment sent for approval.", "carspot-rest-api");
			}			
		}
		else
		{
			$arr['comments']  =  array();
			$success = false;
			$message = __("Some Error Ocuured While Posting Ad", "carspot-rest-api");
			
		}
		$extra['message'] = __("You are posting too fast. Please slow down.", "carspot-rest-api"); 
...
if( !function_exists('carspotAPI_get_post_comments_api' ) )
{
	function carspotAPI_get_post_comments_api( $request  )
	{		
		$json_data = $request->get_json_params();
		$post_id = (isset( $json_data['post_id'] )) ?  $json_data['post_id'] : '';			
		if ( get_query_var( 'paged' ) ) {
...
function carspotAPI_parent_comment_counter($id){
    global $wpdb;
