...
if (!function_exists('carspotAPI_messages_chat_get'))
{
	function carspotAPI_messages_chat_get( $request )
	{
		...
		$posts_per_page = 15;
		$start = ($paged-1) * $posts_per_page;
		$authors	=	( $type == 'sent' ) ? array( $receiver_id, $user_id ) : array( $sender_id, $user_id );
		...
		$max_num_pages 	= ceil($total_posts/$posts_per_page);					
		$args = array( 'author__in' => $authors, 'post_id' => $ad_id, 'parent' => $queryID, 'orderby' => 'comment_date', 'order' => 'DESC', 'paged' => $paged, 'offset' => $start, 'number' => $posts_per_page, );
		$comments				=	get_comments( $args );
		$chat 					= array();
		$chatHistory 			= array();
		$success 				= false;
...
if (!function_exists('carspotAPI_messages_chat_submit_popup'))
{
	function carspotAPI_messages_chat_submit_popup( $request )
	{
		...
		{
			$success  = true;
			$message2 = __("Please Login to send message", "carspot-rest-api");
		}
...
if (!function_exists('carspotAPI_add_messages_get'))
{
	function carspotAPI_add_messages_get( $ad_id = '', $queryID = '', $sender_id = '', $receiver_id = '', $type = 'sent', $message = '' )
	{
		...
		{
			$messageString =  __( "You can not send a message to yourself", 'carspot-rest-api' );	
		}
