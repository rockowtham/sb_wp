...
if (!function_exists('carspotAPI_get_adAddress'))
{
	function carspotAPI_get_adAddress($ad_id = '')
	{
		...
		if( isset($ad_locs) && count($ad_locs) > 0) { $poster_location = implode(", ",$ad_locs); }	
...
if (!function_exists('carspotAPI_get_customFields'))
{
	function carspotAPI_get_customFields($ad_id = '')
	{
		...
		$dynamicData['ad_categories'] = $price  	= get_post_meta($ad_id, "_carspot_ad_categories", true);
...
if (!function_exists('carspotAPI_getPostAdFields'))
{
	function carspotAPI_getPostAdFields($field_type = '', $field_type_name = '', $term_type = 'ad_cats', $only_parent = 0, $name = '',$mainTitle = '', $defaultValue = '', $has_page_number = 1, $is_required = false, $update_val = '', $ad_id = '')
	{
		...
		return	array("main_title" => $mainTitle, "field_type" => $field_type, "field_type_name" => $field_type_name,"field_val" => $update_val, "field_name" => "", "title" => $name, "values" => $values_data, "has_page_number" => (string)1, "is_required" => $is_required, "has_cat_template" => $has_cat_template);
...
class carspotAPI_save_term_inapp_extras {
	...
	public function __construct() {
		...
		{			
			...
			{
				add_action( 'ad_cats_add_form_fields',  array( $this, 'carspotAPI_save_term_inapp_create_screen_fields'), 10, 1 );
				add_action( 'ad_cats_edit_form_fields', array( $this, 'carspotAPI_save_term_inapp_edit_screen_fields' ),  10, 2 );
	...
	public function carspotAPI_save_term_inapp_screen_fields( $term, $taxonomy ) {
		...
		// Set default values.
		if( empty( $carspot_inapp_android ) ) $carspot_inapp_android = '';
		if( empty( $carspot_inapp_ios ) ) $carspot_inapp_ios = '';
...
//new carspotAPI_save_term_inapp_extras;



if (!function_exists('carspotAPI_get_ad_image_with_arrangment'))
{
	function carspotAPI_get_ad_image_with_arrangment($post_id = '', $size  = 'both')
	{
		global $carspot_theme;
		$media = array();
		//$media = get_attached_media( 'image', $post_id );		
