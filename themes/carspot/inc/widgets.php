...
<div class="recent-post-box">
    ...
    <div <?php echo esc_attr( $class_title ); ?>>
		...
		<?php echo esc_html(get_the_date(get_option( 'date_format' ), $recent_post->ID ));  ?>
        </p>
    </div>
...
?>
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_html__( 'Recent Posts', 'carspot' );
		}
...
?>
            ...
            <li> <span class="arrow"><i class="fa fa-angle-down"></i></span>
				...
				if( count((array) $sub_cats ) > 0 )
				{
					echo '<ul class="children-pro">';
					foreach( $sub_cats as $sub_cat )
					{
						$term_get =  get_term($sub_cat->term_id);	
					?>
  
						<li>
                            <a href="<?php echo esc_url( get_term_link( $sub_cat->term_id ) ); ?>">
                                <?php echo esc_html($term_get->name); ?>
...
<?php
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'category_widget_title' ] ) ) {
			...
			$category_widget_title = esc_html__( 'Categories', 'carspot' );
		}
		...
		<p>
			...
			<?php echo esc_html__( 'Hide empty categories?', 'carspot' ); ?>
            </label> 
