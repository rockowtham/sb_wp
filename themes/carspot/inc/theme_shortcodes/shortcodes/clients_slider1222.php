...
require trailingslashit( get_template_directory () ) . "inc/theme_shortcodes/shortcodes/layouts/header_layout.php";
	...
	if( count((array) $rows ) > 0 )
	{
		...
		foreach($rows as $row )
		{
			...
			{
				$clients_thumb =  carspot_returnImgSrc($row['clients_thumb'], '');
				$testimonials_loop .= '<div class="client-logo">
                              <a href="#"><img src="'.esc_url($clients_thumb).'" class="img-responsive" alt="'.esc_html__("clients", "carspot").'" /></a>
                           </div>';
			}
	...
	return '<section class="client-section '.$parallex.' '.$bg_color.' '.$top_padding.'" ' . $style .'>
            <div class="container">
               <div class="row">
